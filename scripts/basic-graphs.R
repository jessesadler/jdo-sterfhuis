...
accounts <- select(accounts, id, label = account_name_desc)

sterfhuis <- graph_from_data_frame(d = transactions, vertices = accounts, directed = TRUE)

set_graph_style()

ggraph(sterfhuis) + geom_edge_link(aes(alpha = count)) + geom_node_point(alpha = 0.7) + theme(legend.position = "none")
