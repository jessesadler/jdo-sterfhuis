...
maternal_capital <- tibble(id = maternal_accounts,
  ...
  mutate(denarii = deb_lsd_d(l, s, d),
         pct = round(denari*100/sum(denarii), 4))

movable_capital_1578 <- bind_rows(paternal_capital, maternal_capital) %>% 
  mutate(pct = round(denari*100/sum(denarii), 4))

total_denari_1578 <- sum(movable_capital_1578$denarii)
movable_capital_1578_total <- tibble(account = "Total 1578",
                                     l = deb_d_librae(total_denari_1578),
                                     s = deb_d_solidi(total_denari_1578),
                                     d = deb_d_denari(total_denari_1578),
                                     date = ymd("1578-12-31"),
                                     denarii = total_denari_1578)
...
maternal_profits1 <- filter(winninge_verlies_deb1, from %in% maternal_accounts) %>% 
  select(id = from, account:denarii) %>% 
  mutate(pct = round(denari*100/sum(denarii), 4)) %>% 
...
maternal_1578 <- deb_lsd_sum(maternal_capital) %>% 
  mutate(denarii = deb_lsd_d(l, s, d)) %>% 
...
maternal_profits_sum1 <- deb_lsd_sum(maternal_profits1) %>% 
  mutate(denarii = deb_lsd_d(l, s, d)) %>% 
...
# 1578
movable_sum_1578 <- deb_lsd_sum(movable_capital_1578) %>% 
  mutate(denarii = deb_lsd_d(l, s, d)) %>% 
  add_column(date = ymd("1578-12-31"))

# 1583
movable_sum_1583 <- deb_lsd_sum(profits_1583) %>% 
  mutate(denarii = deb_lsd_d(l, s, d)) %>% 
  add_column(date = ymd("1583-12-15"))

# 1594
movable_sum_1594 <- deb_lsd_sum(profits_1594) %>% 
  mutate(denarii = deb_lsd_d(l, s, d)) %>% 
...
### Capital and profits in denarii ###
...
paternal_1583_profits <- deb_d_lsd(paternal_1583_profitsd) %>% 
  add_column(denarii = paternal_1583_profitsd,
             date = ymd("1583-12-15"),
             type = "paternal")
...
paternal_1594_profitszsd <- deb_d_lsd(paternal_1594zsd) %>% 
  add_column(denarii = paternal_1594zsd,
             date = ymd("1594-10-01"),
             type = "paternal")
maternal_1594_profitszsd <- deb_d_lsd(maternal_1594zsd) %>% 
  add_column(denarii = maternal_1594zsd,
             date = ymd("1594-10-01"),
             type = "maternal")
